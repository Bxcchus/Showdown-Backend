...
@Component
class MatchRealtimeHub extends TextWebSocketHandler implements HandshakeInterceptor, SubProtocolCapable {

    private static final Logger LOGGER = LoggerFactory.getLogger(MatchRealtimeHub.class);
    private static final String PLAYER_ID = "playerId";
    private final JwtDecoder jwtDecoder;
    private final ObjectMapper objectMapper;
    private final Map<UUID, Set<WebSocketSession>> sessions = new ConcurrentHashMap<>();

    MatchRealtimeHub(JwtDecoder jwtDecoder, ObjectMapper objectMapper) {
    ...
    public boolean beforeHandshake(ServerHttpRequest request, ServerHttpResponse response,
        ...
        try {
            ...
            if (!hasScope(jwt, "match:read")) throw new IllegalArgumentException("Missing match scope");
            attributes.put(PLAYER_ID, UUID.fromString(jwt.getSubject()));
            return true;
        } catch (RuntimeException exception) {
            LOGGER.warn("Match WebSocket handshake rejected: {}", exception.getMessage());
    ...
    @Override public List<String> getSubProtocols() { return List.of("showdown-v1"); }
    ...
    void publishAfterCommit(List<UUID> playerIds, String type) {
    ...
    private static String bearerProtocol(List<String> headers) {
        return headers.stream().flatMap(value -> List.of(value.split(",")).stream())
                ...
                .map(value -> value.substring("bearer.".length())).findFirst()
